...
\begin{document}
    ...
    There's a standard choice, we use \(A = 10\), \(B = 11\), \(C = 12\), \(D = 13\), \(E = 14\), and \(F = 15\).
    
    For example, consider the hexadecimal \(7AF_{16}\).
    We can convert this to decimal as follows:
    \begin{equation}
        7AF_{16} = 7 \cdot 16^2 + A \cdot 16^1 + F \cdot 16^0 = 3 \cdot 16^2 + 10 \cdot 16^1 + 15 \cdot 16^0 = 1967_{10}.
    \end{equation}
    ...
    \begin{app}{}{}
        ...
        For example, if we take \textcolor{glasgowPillarbox}{this red colour} it's represented by \(\#B30C00\) (the \# symbol is commonly used to mean that a number is base 16 in computer science).
        This isn't actually one number, it's three, \(B3_{16}\), \(0C_{16}\) and \(00_{16}\), representing the amount of red, green, and blue respectively.
        That is, we have set the red channel to \(B3_{16} = 11 \cdot 16^1 + 3 \cdot 16^0 = 179_{10}\) out of \(255_{10}\), which is about \qty{70}{\percent}.
        We've set the green channel to \(C_{16} = 12 \cdot 16^0 = 12_{10}\) out of \(255_{10}\), which is about \qty{4}{\percent}.
        Finally, we've set the blue channel to \(0_{16} = 0_{10}\) out of \(255_{10}\), which is to say, it's off.
        
        Note that the brightest we can set any single colour channel is \(FF_{16} = 15 \cdot 16^1 + 15 \cdot 16^0 = 255\).
    \end{app}
    
    \begin{problem}{}{}
        Consider the colour \(\#23F089\), which of the following do you think this is?
        \tikzexternaldisable
        \begin{enumerate}
            \item 
